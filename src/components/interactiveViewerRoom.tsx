...
import AngleTool, { AngleToolRenderer } from './AngleTool';
import * as THREE from 'three';
import { MeshLine, MeshLineMaterial } from 'three.meshline';
import { calculate2DPolygonArea } from './AreaTool';
...
const InteractiveViewerRoom: React.FC = () => {
  ...
  const {
    ...
    points: areaPoints,
    lines: areaLines,
    currentStartPoint: areaStartPoint,
    currentEndPoint: areaEndPoint,
    isPolygonClosed,
    handleAreaClick,
    handleAreaPointerMove,
    dynamicAreaLineGeometry,
    isHoveringThreshold,
  ...
  const {
    ...
    points: anglePoints,
    lines: angleLines,
    currentDynamicPoint: angleDynamicPoint,
    angle,
    handleAddPoint: handleAngleAddPoint,
    handleDynamicPointUpdate: handleAngleDynamicUpdate,
  } = AngleTool();
  ...
  const LengthMode = () => {
    toggLengthMode();
    setIsLengthModalVisible(!isLengthModalVisible);
    setCurrentStep(1);
  ...
  const closeModal = () => {
    ...
    setIsLengthModalOpened(true);
  };

  const lineGeometry = useMemo(() => {
    const geometry = new THREE.BufferGeometry();

    if (activeLineStart && activeLineEnd) {
        const pointsArray = [activeLineStart, activeLineEnd];
        geometry.setFromPoints(pointsArray);
    } else {
        // Set empty geometry for fallback
        geometry.setFromPoints([]);
    }

    return geometry;
  }, [activeLineStart, activeLineEnd]);

  const dynamicLineGeometry = useMemo(() => {
    const geometry = new THREE.BufferGeometry();
    if (activeLineStart && activeLineEnd) {
        geometry.setFromPoints([activeLineStart, activeLineEnd]);
    } else {
...
// Finalized static lines are handled consistently
const staticLinesGeometry = useMemo(() => {
...
const staticLineData = useMemo(() => {
  return lines.map(([start, end]) => {
      const geometry = new THREE.BufferGeometry();
      geometry.setFromPoints([start, end]); // Define geometry for each line
      ...
      const distance = start.distanceTo(end).toFixed(2); // Precompute distance

      return { geometry, midpoint, distance }; // Return geometry, midpoint, and distance
  });
}, [lines]);
    ...
    <div className="w-full max-w-screen-3xl bg-white rounded-md shadow-default dark:bg-boxdark dark:text-white p-4 mx-auto mt-6">
      ...
      <div ref={viewerRef} className="relative flex w-full h-[70vh] mt-4 bg-gray-700 rounded-lg overflow-hidden shadow-lg">
        ...
        <Canvas
          ...
          onPointerDown={(event) => {
              if (camera && scene) {
                handlePointerDown(event, camera, scene); // Marker logic

                if (isLengthMode) {
                  handleLengthClick(event, camera, scene); // Call the method from LengthTool
                }
                if (isAreaMode) {
                  handleAreaClick(event, camera, scene);
                }
                if (isAngleMode) {
                  const raycaster = new THREE.Raycaster();
                  const mouse = new THREE.Vector2();
              
                  // Safely cast event.target to HTMLElement
                  ...
                  if (intersects.length > 0) {
                      ...
                      handleAngleAddPoint(new THREE.Vector3(point.x, point.y, point.z));
                  }
              }
          ...
          onPointerMove={(event) => {
              if (camera && scene) {
                handlePointerMove(event, camera, scene); // Marker logic

                if (isLengthMode && activeLineStart) {
                  handleLengthPointerMove(event, camera, scene); //LengthTool Logic
                }
                if (isAreaMode) {
                  handleAreaPointerMove(event, camera, scene); // Area Logic
                }
                if (isAngleMode) { //Angle Logic
                  const raycaster = new THREE.Raycaster();
                  const mouse = new THREE.Vector2();
                  ...
                  // Check if the target is a valid HTMLElement before proceeding
                  if (target && target.getBoundingClientRect) {
                      ...
                      if (intersects.length > 0) {
                          const point = intersects[0].point;
                          handleAngleDynamicUpdate(new THREE.Vector3(point.x, point.y, point.z));
                      }
                  }
                }
              }
        ...
        >
            ...
            {isLengthMode && (
              <>
                ...
                {/* Dynamic start point for Length */}
                {activeLineStart && (
                ...
                {/* Dynamic end point for Length */}
                {activeLineEnd && (
                  <mesh position={[activeLineEnd.x, activeLineEnd.y, activeLineEnd.z]}>
                    <sphereGeometry args={[3, 64, 64]} />
                    <meshBasicMaterial color="blue" />
                  </mesh>
                )}

                {/* Dynamic Line Measurement */}
                {/* Dynamic Measurement */}
                {activeLineStart && activeLineEnd && (
                  <Html
                      position={[
                          (activeLineStart.x + activeLineEnd.x) / 2,
                          (activeLineStart.y + activeLineEnd.y) / 2,
                          (activeLineStart.z + activeLineEnd.z) / 2,
                      ]}
                      center
                  >
                      <div
                      ...
                      >
                          {isCalibrated
                              ? `${convertToRealWorldDistance(activeLineStart.distanceTo(activeLineEnd)).toFixed(2)} meters`
                              : `${activeLineStart.distanceTo(activeLineEnd).toFixed(2)} units`}
                      </div>
                  </Html>
                )}

                {/* Static Lines with Measurement */}
                {!isPromptOpen && staticLineData.map(({ geometry, midpoint, distance }, index) => {
                  const start = lines[index]?.[0] || { x: 0, y: 0, z: 0 };
                  const end = lines[index]?.[1] || { x: 0, y: 0, z: 0 };

                  // Ensure distance is a number
                  const realDistance = convertToRealWorldDistance(Number(distance));
                  ...
                  return (
                      <React.Fragment key={index}>
                          ...
                          <Html position={[midpoint.x, midpoint.y, midpoint.z]} center>
                              ...
                              >
                                  {isCalibrated && realDistance !== null
                                      ? `${realDistance.toFixed(2)} meters`
                                      : `${Number(distance).toFixed(2)} units`}
                              </div>
                          </Html>
                      </React.Fragment>
                  );
            ...
            {isAreaMode && (
              <>
               {/* Dynamic Line Rendering */}
                <line>
                    <primitive attach="geometry" object={dynamicAreaLineGeometry} />
                    <lineBasicMaterial color="green" />
                </line>
                {/* Dynamic Line Measurement Display */}
                {areaStartPoint && areaEndPoint && (
                    <Html
                        position={[
                            (areaStartPoint.x + areaEndPoint.x) / 2,
                            (areaStartPoint.y + areaEndPoint.y) / 2,
                            (areaStartPoint.z + areaEndPoint.z) / 2,
                        ]}
                        center
                    >
                        <div
                        ...
                        >
                            {isCalibrated 
                              ?`${convertToRealWorldDistance(areaStartPoint.distanceTo(areaEndPoint)).toFixed(2)} meters`
                              :`${areaStartPoint.distanceTo(areaEndPoint).toFixed(2)} units`
                              }
                        </div>
                    </Html>
                )}
                ...
                {/* Dynamic Start Point */}
                {areaStartPoint && (
                    <mesh position={[areaStartPoint.x, areaStartPoint.y, areaStartPoint.z]}>
                        <sphereGeometry args={[3, 64, 64]} />
                ...
                {/* Dynamic End Point */}
                {areaEndPoint && (
                    <mesh position={[areaEndPoint.x, areaEndPoint.y, areaEndPoint.z]}>
                        <sphereGeometry args={[3, 64, 64]} />
                        <meshBasicMaterial color="green" />
                    </mesh>
                )}

                {/* Static Lines and Measurements */}
                {areaLines.map(([start, end], index) => (
                  <React.Fragment key={index}>
                    <line>
                    ...
                    {/* Static Line Start Point */}
                    <mesh position={[start.x, start.y, start.z]}>
                      <sphereGeometry args={[3, 64, 64]} />
                      <meshBasicMaterial color="green" />
                    </mesh>

                    {/* Static Line End Point */}
                    <mesh position={[end.x, end.y, end.z]}>
                      <sphereGeometry args={[3, 64, 64]} />
                      <meshBasicMaterial color="green" />
                    </mesh>

                    {/* Measurement for Static Line */}
                    <Html
                      position={[
                        (start.x + end.x) / 2,
                        (start.y + end.y) / 2,
                    ...
                    >
                      ...
                      >
                        {isCalibrated 
                          ?`${convertToRealWorldDistance(start.distanceTo(end)).toFixed(2)} meters`
                          :`${start.distanceTo(end).toFixed(2)} units`}
                      </div>
                    </Html>
                  </React.Fragment>
                ))}

                {/* Starting Point close polygon Feedback */}
                {areaPoints[0] && (
                    <mesh position={[areaPoints[0].x, areaPoints[0].y, areaPoints[0].z]}>
                        <sphereGeometry args={[isHoveringThreshold ? 6 : 3, 64, 64]} /> 
                        <meshBasicMaterial color={isHoveringThreshold ? "red" : "green"} />
                    </mesh>
                )}
                {/* Added Static Area Display */}
                {isPolygonClosed && areaPoints.length > 2 && (
                  <Html
                    position={[
                      areaPoints[0].x,
                      areaPoints[0].y,
                  ...
                  >
                    ...
                    >
                      {/* Area Display */}
                      {isCalibrated 
                        ?`Area: ${convertToRealWorldDistance(calculate2DPolygonArea(areaPoints.map((p) => new THREE.Vector2(p.x, p.y)))).toFixed(2)} meters²`
                        :`Area: ${calculate2DPolygonArea(areaPoints.map((p) => new THREE.Vector2(p.x, p.y))).toFixed(2)} units²`}
                    </div>
                  </Html>
                )}
              </>
            ...
            {isAngleMode && (
              <AngleToolRenderer
                  ...
                  scene={scene as THREE.Scene} // Type assertion
                  camera={camera as THREE.Camera} // Type assertion
              />
            )}

            {/* Marker Render SECTION */}
            {/* Render finalized paths for MArker */}
            {drawingPaths.map((path, pathIndex) => {
              const meshLine = new MeshLine();
              meshLine.setPoints(path.flatMap((p) => [p.x, p.y, p.z])); // Flatten vector array to [x, y, z, ...]
              ...
              return (
                <primitive
                  object={new THREE.Mesh(meshLine, material)}
                  key={pathIndex}
                />
              );
            })}
            {/* Render active path dynamically for MArker */}
            <line>
              <bufferGeometry ref={activePathRef} />
        ...
        <div className="flex flex-col space-y-4 p-2 right-3 top-40 transform -translate-y-1/2 absolute bg-white dark:bg-gray-700 rounded-lg shadow-lg">
          {/* Area Measure Icon */}
          <button 
            className="bg-primary text-white p-2 rounded-full shadow-lg hover:bg-opacity-80 transition"
            onClick={AreaMode}
            >
      ...
      {/* Length tutorial steps modal, DON'T FORGET */}
      {isLengthModalVisible && isLengthMode && !isLengthModalOpened &&(
         ...
         <div className="bg-white dark:bg-gray-900 p-6 rounded-lg shadow-lg max-w-md w-full relative">
           ...
           <div className='flex justify-end mb-5'>
            <button
             onClick={closeModal}
             className="absolute top-5 right-6 text-gray-500 dark:text-gray-400 hover:text-gray-900 dark:hover:text-gray-600 text-sm"
            >
              Skip
            </button>
           </div>
           ...
           {currentStep === 1 && (
             <>
               <h2 className="text-xl font-semibold mb-4 text-gray-900 dark:text-gray-200">
                 Step 1: Measure a Known Object
               </h2>
               <p className="text-sm mb-6 text-gray-700 dark:text-gray-300">
                 Use the length tool to measure a known object in the scene. For example, you can measure the height of a door or the width of a table.
               </p>
               <div className="flex justify-end">
                 ...
                 >
                   <span>Step 2</span>
                   <span className="ml-1">&rarr;</span> {/* Right Arrow */}
                 </button>
               </div>
             </>
           )}
           ...
           {currentStep === 2 && (
             <>
               <h2 className="text-xl font-semibold mb-4 text-gray-900 dark:text-gray-200">
                 Step 2: Input the Real-World Measurement
               </h2>
               <p className="text-sm mb-6 text-gray-700 dark:text-gray-300">
                 After measuring the object, input its real-world measurement in meters. This will calibrate the tool for accurate measurements.
               </p>
      ...
      {/*Length real measurement input modal */}
      {isPromptOpen && (
       <div className="fixed inset-0 bg-gray-900 bg-opacity-60 flex justify-center items-center z-[999999]">
       <div className="bg-white dark:bg-gray-800 p-8 rounded-md shadow-2xl max-w-lg w-full transform scale-105 transition-transform duration-300">
         <h3 className="text-xl font-semibold text-gray-800 dark:text-gray-100 mb-4">Set Real Measurement</h3>
         <p className="text-sm text-gray-700 dark:text-gray-300 mb-6">
           The tool measured a virtual distance of <strong>{virtualMeasurement} units</strong>. Please enter the corresponding real-world measurement below to calibrate.
         </p>
         <div className="mb-6">
            <div className="relative">
              <input
                ...
                className="w-full px-4 py-2 border border-gray-300 dark:border-gray-600 rounded-md focus:ring focus:ring-indigo-500 focus:outline-none dark:bg-gray-700 dark:text-white appearance-none"
                placeholder="Measurement in meters"
              />
              {/* Custom Buttons */}
              <div className="absolute inset-y-0 right-0 flex flex-col items-center justify-center">
                <button
                  type="button"
                  onClick={() => setRealMeasurement((prev) => String(Number(prev || 0) + 1))}
                  className="bg-gray-200 dark:bg-gray-700 text-gray-700 dark:text-gray-300 w-8 h-8 flex items-center justify-center rounded-tr-md hover:bg-gray-300 dark:hover:bg-gray-600"
                >
                  <ChevronUpIcon className="w-5 h-5 text-gray-700 dark:text-gray-300" />
                </button>
                <button
                  type="button"
                  onClick={() => setRealMeasurement((prev) => String(Math.max(Number(prev || 0) - 1, 0)))}
                  className="bg-gray-200 dark:bg-gray-700 text-gray-700 dark:text-gray-300 w-8 h-8 flex items-center justify-center rounded-br-md hover:bg-gray-300 dark:hover:bg-gray-600"
                >
         ...
         {/* Validation Message */}
         {validationError && (
           <div className="text-red-500 text-sm mb-4 -mt-4.5 ml-1">
             {validationError}
           </div>
         )}
         <div className="flex justify-between">
           <button
             onClick={() => {
               setRealMeasurement('');
               setValidationError(''); // Clear the error when canceled
               setIsPromptOpen(false);
             }}
             className="bg-gray-200 dark:bg-gray-700 text-gray-700 dark:text-gray-300 py-2 px-4 rounded-lg shadow hover:bg-gray-300 dark:hover:bg-gray-600"
           >
             Cancel
           </button>
           <button
             onClick={() => {
               if (!realMeasurement || parseFloat(realMeasurement) <= 0) {
                 setValidationError('*please enter a positive number.');
               } else {
                 handleRealMeasurementSubmit();
                 setValidationError(''); // Clear the error on successful submission
               }
             }}
